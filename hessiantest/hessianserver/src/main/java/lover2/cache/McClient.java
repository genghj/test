package lover2.cache;

import  lover2.MemcachedbFactory;
...
public class McClient {
    ...
    public static void mDelete(String key) {
        if(MemcachedbFactory.isOk) {
            int memNum = MemcachedbFactory.memCacheNum;
            if(memNum > 0) {
                long keyId = (long)(StringKeyUtil.getUserId(key) % memNum);
    ...
    public static void mPut(String key, Object obj, long times) {
        if(MemcachedbFactory.isOk) {
            if(times >= 2592000000L) {
                times = 2591999000L;
            }
    ...
    public static boolean mExists(String key) {
        if(!MemcachedbFactory.isOk) {
            return false;
        } else {
            int memNum = MemcachedbFactory.memCacheNum;
